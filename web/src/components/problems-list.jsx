...
export function ProblemList() {
	...
	const handleRowClick = (problem) => {
		window.vscode.postMessage({ command: "open-problem", problem });
	};
	...
	return (
		<div className='p-4 bg-[#1e1e1e] text-gray-200 font-inter rounded-lg h-full flex flex-col'>
					...
					<tbody>
						{items.map((q, idx) => (
							<tr
								...
								className={`border-t border-gray-800 hover:bg-[#252526] transition cursor-pointer ${
									idx % 2 === 0 ? "bg-[#1e1e1e]" : "bg-[#222]"
								}`}
								onClick={() => handleRowClick(q)}
							>
								<td className='py-3 px-4 text-gray-400 align-top'>{q.frontendQuestionId}</td>

								<td className='py-3 px-4'>
									<span className='text-blue-400 font-medium'>{q.title}</span>
								...
								<td className={`py-3 px-4 font-semibold ${getDifficultyClass(q.difficulty)}`}>
									{q.difficulty}
								</td>

								<td className='py-3 px-4 text-gray-400'>{q.acRate.toFixed(1)}%</td>
							</tr>
						))}
					</tbody>
				</table>
			...
			{items.length === 0 && <p className='text-center text-gray-500 mt-6'>Loading problems...</p>}
		</div>
	);
}
